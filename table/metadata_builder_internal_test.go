...
func TestMinimalBuild(t *testing.T) {
	...
	sortOrder := sortOrder()
	formatVersion := 1
	props := map[string]string{PROPERTY_FORMAT_VERSION: "1"}
...
func TestAddRemovePartitionSpec(t *testing.T) {
	...
	require.NoError(t, err)
	// Remove the spec
	err = newBuilder.RemovePartitionSpecs([]int{1})
	require.NoError(t, err)
...
func TestSetDefaultPartitionSpec(t *testing.T) {
	...
	require.NoError(t, err)
	err = builder.AddPartitionSpec(&addedSpec, false)
	require.NoError(t, err)
...
func TestSetExistingDefaultPartitionSpec(t *testing.T) {
	...
	err = newBuilder.SetDefaultSpecID(0)
	require.NoError(t, err)
...
func TestSetBranchSnapshotCreatesBranchIfNotExists(t *testing.T) {
	...
	err := builder.AddSnapshot(&snapshot)
	require.NoError(t, err)
	err = builder.SetSnapshotRef("new_branch", 2, BranchRef)
	require.NoError(t, err)
