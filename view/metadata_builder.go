...
// MetadataBuilder constructs view metadata with validation.
// It tracks changes and ensures all required fields are present and valid.
...
// metadataLocation is the location of the current metadata file (optional).
func MetadataBuilderFrom(base Metadata, metadataLocation string) *MetadataBuilder {
	schemas := make([]*iceberg.Schema, 0)
	schemasById := make(map[int]*iceberg.Schema)
...
func (b *MetadataBuilder) AssignUUID(uuid string) error {
	if uuid == "" {
		return fmt.Errorf("%w: cannot set uuid to empty string", iceberg.ErrInvalidArgument)
	}

	if b.uuid != "" && b.uuid != uuid {
		return fmt.Errorf("%w: cannot reassign uuid from %s to %s", iceberg.ErrInvalidArgument, b.uuid, uuid)
	}
	...
	b.changes = append(b.changes, NewAssignUUIDUpdate(uuid))
	b.uuid = uuid
...
func (b *MetadataBuilder) reuseOrCreateNewVersionID(version *Version) int64 {
	for _, existing := range b.versions {
		if b.sameViewVersion(version, existing) {
...
func (b *MetadataBuilder) sameViewVersion(v1, v2 *Version) bool {
	if v1.SchemaID != v2.SchemaID {
		return false
	}

	if len(v1.Summary) != len(v2.Summary) {
		return false
	}

	for k, v := range v1.Summary {
		if v2.Summary[k] != v {
			return false
		}
	}

	if len(v1.Representations) != len(v2.Representations) {
		return false
	}

	for i, r1 := range v1.Representations {
		r2 := v2.Representations[i]
		if r1.Type != r2.Type || r1.SQL != r2.SQL || r1.Dialect != r2.Dialect {
			return false
		}
	}

	if (v1.DefaultCatalog == nil) != (v2.DefaultCatalog == nil) {
		return false
	}

	if v1.DefaultCatalog != nil && *v1.DefaultCatalog != *v2.DefaultCatalog {
		return false
	}

	if len(v1.DefaultNamespace) != len(v2.DefaultNamespace) {
		return false
	}

	for i, ns := range v1.DefaultNamespace {
		if v2.DefaultNamespace[i] != ns {
			return false
		}
	}
...
func (b *MetadataBuilder) Build() (*ViewMetadataBuildResult, error) {
	if b.metadataLocation != nil && len(b.changes) > 0 {
		return nil, fmt.Errorf("%w: cannot create metadata with both metadata location and changes", ErrInvalidViewMetadata)
	}
...
// NewViewMetadata creates a new view metadata for a new view.
func NewViewMetadata(
	location string,
	schema *iceberg.Schema,
	version *Version,
	properties map[string]string,
) (*MetadataBuilder, error) {
	builder, err := NewMetadataBuilder(DefaultViewFormatVersion)
	if err != nil {
		return nil, err
	}

	if err := builder.AssignUUID(uuid.New().String()); err != nil {
		return nil, err
	}

	if err := builder.SetLocation(location); err != nil {
		return nil, err
	}

	if properties != nil {
		if err := builder.SetProperties(properties); err != nil {
			return nil, err
		}
	}

	if err := builder.SetCurrentVersion(version, schema); err != nil {
		return nil, err
	}
